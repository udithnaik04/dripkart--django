...
def register(request):
    if request.method =='POST':
        ...
        if form.is_valid():
            ...
            send_email.send()
            #messages.success(request,'please check your verification email in your inbox of ')
...
def activate(request,uidb64,token):
    ...
    if user is not None and default_token_generator.check_token(user,token):
        user.is_active = True
        user.save()
        messages.success(request,'congratulations! the registration is successfull')
        return redirect('login')
    else:
